...
const SignalRelay = require('../services/signal-relay');
...
function setupFillNotifications(broadcaster) {
  ...
  broadcaster.handleStreamerMessage = async function(message) {
    ...
    if (fillData && fillBroadcaster) {
      console.log(`📨 Fill detected: ${fillData.symbol} ${fillData.action} ${fillData.filledQuantity || fillData.quantity}`);
      
      // Try to match with original signal ID
      // For now, we'll let the fill broadcaster determine tiers by symbol
      // In production, you'd track orderId -> signalId mapping
      let signalId = null;
      
      // Broadcast to tiered channels (will use signal tier mapping if available)
      await fillBroadcaster.broadcastFill(fillData, signalId);
    }
  };
}
...
myDiscordClient.on('messageCreate', async (message) => {
  try {
    ...
    if (signalRelay) {
      ...
      if (fillBroadcaster) {
        const sentTiers = [];
        if (relayResults.vip?.success) sentTiers.push('vip');
        if (relayResults.premium?.success) sentTiers.push('premium');
        if (relayResults.basic?.success) sentTiers.push('basic');
        
        if (sentTiers.length > 0) {
          fillBroadcaster.trackSignalTiers(signal.id, sentTiers);
          console.log(`📋 Tracked signal ${signal.id} → tiers: ${sentTiers.join(', ')}`);
        }
      }
    }
...
/**
 * Process fill from Tastytrade account streamer
 * Extracts fill data and broadcasts to appropriate tiers
 */
function processFillFromStreamer(streamerMessage) {
  try {
    // Parse streamer message to extract fill data
    const fill = parseFillFromStreamer(streamerMessage);
    ...
    // Try to match with original signal ID
    // Look for order ID or symbol match in recent signals
    let signalId = null;
    
    // If we can extract order ID from the fill, try to match it
    // This is a simplified matching - you may need to enhance based on your order tracking
    if (fill.orderId) {
      // Try to find matching signal by order ID or symbol
      // In a real implementation, you'd track orderId -> signalId mapping
      signalId = findSignalIdByOrderId(fill.orderId, fill.symbol);
    }
    
    // Broadcast fill to appropriate tiers
    if (fillBroadcaster) {
      fillBroadcaster.broadcastFill(fill, signalId);
    }
    
  } catch (error) {
    console.error('❌ Error processing fill from streamer:', error);
  }
}

/**
 * Parse fill data from Tastytrade account streamer message
 */
function parseFillFromStreamer(message) {
  try {
    // Tastytrade streamer messages have different formats
    ...
    // Check if this is a fill/order event
    if (data.type !== 'Order' && data.type !== 'Fill' && !data['order-status']) {
      return null;
    }
    
    // Extract fill information
    const fill = {
      orderId: data['order-id'] || data.id || data['orderId'],
      symbol: data.symbol || data['underlying-symbol'],
      action: data.action,
      quantity: data.quantity || data.size,
      filledQuantity: data['filled-quantity'] || data.filledQuantity || data.quantity,
      totalQuantity: data['total-quantity'] || data.totalQuantity || data.quantity,
      fillPrice: data['fill-price'] || data.fillPrice || data.price,
      price: data.price || data['fill-price'],
      instrumentType: data['instrument-type'] || data.instrumentType || 'Equity',
      status: data['order-status'] || data.status || 'Filled',
      fees: data.fees || 0,
      commission: data.commission || 0,
      accountNumber: data['account-number'] || data.accountNumber,
      executionVenue: data['execution-venue'] || data.executionVenue || data.exchange,
      filledAt: data['filled-at'] ? new Date(data['filled-at']) : new Date(),
      timestamp: new Date()
    };
    
    // Extract option details if available
    if (data['strike-price'] || data.strike) {
      fill.strike = data['strike-price'] || data.strike;
    }
    
    if (data['expiration-date'] || data.expiration) {
      fill.expiration = data['expiration-date'] || data.expiration;
    }
    
    if (data['option-type'] || data.optionType) {
      fill.optionType = data['option-type'] || data.optionType;
    }
    ...
    console.error('❌ Error parsing fill from streamer:', error);
    return null;
  }
}

/**
 * Find signal ID by order ID or symbol
 * This is a simplified implementation - enhance based on your tracking needs
 */
function findSignalIdByOrderId(orderId, symbol) {
  // In a real implementation, you'd maintain a mapping of orderId -> signalId
  // For now, return null and let fill broadcaster determine tiers by symbol
  return null;
}
...
process.on('SIGINT', async () => {
  ...
  if (fillBroadcaster) {
    console.log('Clearing fill history...');
    fillBroadcaster.clearHistory();
  }
